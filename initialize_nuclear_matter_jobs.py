...
with open(file_path,'w') as f_1:
    f_1.write('export OMP_NUM_THREADS=8'+'\n\n')
    for loop1 in range(9):
        for loop2 in range(13):
            cD = -3 + loop2 * 0.5
            cE = -1 + loop1 * 0.25
            for loop3 in range(5):
                ...
                f_1.write('/home/g1u/sw/intel_openmpi/bin/mpiexec -np 8 ./prog_ccm.exe '+ccm_in_file_path+' > '+ccm_out_file_path+'\n')
                f_1.write('wait'+'\n')
...
with open(file_path,'w') as f_1:
    ...
    for loop1 in range(9):
        for loop2 in range(13):
            ...
            for loop3 in range(5):
                density = 0.15 + loop3 * 0.02
                ccm_in_file_path = './ccm_in_'+matter_type+'_%d_cD_%.2f_cE_%.2f_rho_%.2f' % (neutron_num,cD,cE,density)
                ccm_out_file_path = './output/'+matter_type+'_%d_cD_%.2f_cE_%.2f_rho_%.2f.out &' % (neutron_num,cD,cE,density)
                f_1.write('/home/g1u/sw/intel_openmpi/bin/mpiexec -np 8 ./prog_ccm.exe '+ccm_in_file_path+' > '+ccm_out_file_path+'\n')
                f_1.write('wait'+'\n')
